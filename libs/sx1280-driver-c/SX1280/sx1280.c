...
uint16_t SX1280GetFirmwareVersion(void) {
    return (((SX1280HalReadRegister(REG_LR_FIRMWARE_VERSION_MSB)) << 8)
            | (SX1280HalReadRegister(REG_LR_FIRMWARE_VERSION_MSB + 1)));
}
...
void SX1280GetPacketStatus(PacketStatus_t *pktStatus) {
    ...
    case PACKET_TYPE_RANGING:
        ...
        (status[1] < 128) ? (pktStatus->Params.LoRa.SnrPkt = status[1] / 4)
...
void SX1280Calibrate(CalibrationParams_t calibParam) {
    uint8_t cal = (calibParam.ADCBulkPEnable << 5) | (calibParam.ADCBulkNEnable << 4) | (calibParam.ADCPulseEnable << 3)
                  | (calibParam.PLLEnable << 2) | (calibParam.RC13MEnable << 1) | (calibParam.RC64KEnable);
...
void SX1280EnableManualGain(void) {
    SX1280HalWriteRegister(REG_ENABLE_MANUAL_GAIN_CONTROL,
                           SX1280HalReadRegister(REG_ENABLE_MANUAL_GAIN_CONTROL) | MASK_MANUAL_GAIN_CONTROL);
...
void SX1280SetManualGainValue(uint8_t gain) {
    SX1280HalWriteRegister(REG_MANUAL_GAIN_VALUE,
                           (SX1280HalReadRegister(REG_MANUAL_GAIN_VALUE) & MASK_MANUAL_GAIN_VALUE) | gain);
}
...
void SX1280SetRangingIdLength(RadioRangingIdCheckLengths_t length) {
    switch (SX1280GetPacketType()) {
    case PACKET_TYPE_RANGING:
        SX1280HalWriteRegister(REG_LR_RANGINGIDCHECKLENGTH,
                               ((((uint8_t)length) & 0x03) << 6)
                                 | (SX1280HalReadRegister(REG_LR_RANGINGIDCHECKLENGTH) & 0x3F));
        break;
...
double SX1280GetRangingResult(RadioRangingResultTypes_t resultType) {
    ...
    case PACKET_TYPE_RANGING:
        ...
        SX1280HalWriteRegister(REG_LR_RANGINGRESULTCONFIG,
                               (SX1280HalReadRegister(REG_LR_RANGINGRESULTCONFIG) & MASK_RANGINGMUXSEL)
                                 | ((((uint8_t)resultType) & 0x03) << 4));
        valLsb = ((SX1280HalReadRegister(REG_LR_RANGINGRESULTBASEADDR) << 16)
                  | (SX1280HalReadRegister(REG_LR_RANGINGRESULTBASEADDR + 1) << 8)
                  | (SX1280HalReadRegister(REG_LR_RANGINGRESULTBASEADDR + 2)));
...
void SX1280RangingSetFilterNumSamples(uint8_t num) {
    ...
    SX1280HalWriteRegister(REG_LR_RANGINGFILTERWINDOWSIZE,
...
double SX1280GetRangingCorrectionPerSfBwGain(const RadioLoRaSpreadingFactors_t sf,
                                             const RadioLoRaBandwidths_t bw,
                                             const int8_t gain) {
    uint8_t sf_index = 0, bw_index = 0;
...
double SX1280ComputeRangingCorrectionPolynome(const RadioLoRaSpreadingFactors_t sf,
                                              const RadioLoRaBandwidths_t bw,
                                              const double median) {
    uint8_t sf_index = 0, bw_index = 0;
...
void SX1280OnDioIrq(void) {
     ...
     * on radio interrupt.
     *
     * Exception on this function? Make sure your callback pointers are still valid
     */
    if (PollingMode == true) {
        IrqState = true;
    } else {
